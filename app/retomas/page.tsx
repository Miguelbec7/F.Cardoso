...
export const metadata: Metadata = {
  title: "Retomas",
  description:
    "Aceitamos o seu carro como retoma. Envie os dados e fotografias por WhatsApp e receba uma avaliação rápida.",
};

const PASSOS = [
  { titulo: "Envie os dados", texto: "Marca, modelo, ano, quilómetros e algumas fotografias, por WhatsApp." },
  { titulo: "Avaliação rápida", texto: "Normalmente respondemos com uma estimativa em poucas horas." },
  { titulo: "Combinamos a retoma", texto: "O valor entra como parte do pagamento do automóvel que escolher." },
];

export default function RetomasPage() {
  const { whatsappNumber } = getSiteSettings();
  const waHref = whatsappLink("Olá, gostaria de saber mais sobre a retoma do meu carro.", whatsappNumber);

  return (
    <main className="mx-auto max-w-3xl px-5 py-14">
      <span className="text-[0.72rem] font-bold tracking-[0.16em] text-brand-bright uppercase">Retomas</span>
      <h1 className="mt-2 text-[clamp(1.8rem,5vw,2.6rem)] font-extrabold tracking-tight text-balance">
        Aceitamos o seu carro como retoma
      </h1>
      <p className="mt-4 max-w-[60ch] text-ink-dim">
        Está a pensar trocar de carro? O seu automóvel atual pode entrar como parte do pagamento. Avaliamos
        qualquer marca e modelo, independentemente do estado.
      </p>
